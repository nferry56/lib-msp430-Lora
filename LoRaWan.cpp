...
*/
#ifdef EU863
const unsigned char LoRaWan::LoRa_Frequency[8][3] = {
...
/*
*****************************************************************************************
* Description: S_Table used for AES encryption
*****************************************************************************************
*/
const unsigned char LoRaWan::S_Table[16][16] = {
...
bool LoRaWan::begin()
{
    return begin( _EU863, CH0, SF7BW125, 14, false);
}
/**************************************************************************/
 /*!
     @brief  Initializes the LoRa module, including configuring SPI, configuring
             the frameCounter and txrandomNum.
     @return True if the LoRa module has been initialized
 */
 /**************************************************************************/
bool LoRaWan::begin( lrw_region_t region, lrw_channel_t channel, lrw_datarate_t datarate, uint8_t txpower, bool confirmUplink )
{
    // Check parameters
    ...
    // init LoRaWan Parameters
    _class = CLASS_A;
    _frameCounter = 0x00;
    _txrandomNum = 0x00;

    // Initialize the LoRa radio module (with default values)
    if( lora.begin(868000000, 7, 125000, 5, 14) )
        return -1;
...
/**************************************************************************/
/*!
    @brief Sets the LoRAWan datarate
    @param datarate Bandwidth and Frequency plan.
*/
/**************************************************************************/
int LoRaWan::setDatarate(lrw_datarate_t datarate) {
  if( datarate < 0 || datarate > FSCK )
      return -1;

  _datarate = datarate;
  if( _datarate <= SF7BW125 ) {
      lora.setSpreadingFactor( 12 - _datarate );
      lora.setSignalBandwidth( 125000 );
  } else if( _datarate == SF7BW250 ) {
      lora.setSpreadingFactor( 7 );
      lora.setSignalBandwidth( 250000 );
  } else if( _datarate == FSCK ) {
      // Unsupported yet //
      return -1;
  }
...
/**************************************************************************/
/*!
    @brief Sets the LoRaWan channel.
    @param channel Which channel to send data
*/
/**************************************************************************/
int LoRaWan::setChannel(lrw_channel_t channel) {
  if( channel < 0 || channel > MULTI )
      return -1;

  _channel = channel;
  _isMultiChan = (_channel == MULTI);
  lora.setFrequency( LoRa_Frequency[_channel][0],
                     LoRa_Frequency[_channel][1],
                     LoRa_Frequency[_channel][2] );
...
/*!
    ...
    // Valid values in dBm are: 0-20 dBm
*/
/**************************************************************************/
int LoRaWan::setPower(int8_t txpower) {
    if( txpower < 0 || txpower > 20 )
        return -1;

    _txpower = txpower;
    lora.setTxPower( _txpower, (_txpower > 15)? PA_OUTPUT_PA_BOOST_PIN : PA_OUTPUT_RFO_PIN );
...
/**************************************************************************/
/*!
    @brief  Sends a package with the LoRa module module.
    @param    *buffer
...
*/
/**************************************************************************/
void LoRaWan::send_Packet(unsigned char *buffer, unsigned char size)
{
  // Set Lora in Standby mode wait on mode ready
  while( !lora.beginPacket() ) {
      // yield();
  }

  // Change LoRa frequency channel (ChannelHopping) in multi-channels
  // Don't change in mono channel mode
  if (_isMultiChan == 1) {
    lora.setFrequency( LoRa_Frequency[_channel][0], LoRa_Frequency[_channel][1], LoRa_Frequency[_channel][2] );
  } else // Single Channel
  if( _isAdaptativeDatarate ) {
      // Set for Adaptative Datarate //
      //lora.setSpreadingFactor( _sf );
      //lora.setSignalBandwidth( _bw );
  }
...
*/
/**************************************************************************/
void LoRaWan::sendData(uint8_t Frame_Port, const uint8_t *Data, const uint8_t Data_Length )
{
  //Define variables
  unsigned char i;
  ...
  unsigned char LORA_Data[64];
  unsigned char LORA_Package_Length;
  ...
  Encrypt_Payload(tmpData, Data_Length, _frameCounter, Direction);

  // Build LoraWan Packet
  LORA_Data[0] = Mac_Header;
  LORA_Data[1] = DevAddr[3];
  LORA_Data[2] = DevAddr[2];
  LORA_Data[3] = DevAddr[1];
  LORA_Data[4] = DevAddr[0];
  LORA_Data[5] = Frame_Control;
  LORA_Data[6] = (_frameCounter & 0x00FF);
  LORA_Data[7] = ((_frameCounter >> 8) & 0x00FF);
  LORA_Data[8] = Frame_Port;

  // Set Current package length
  LORA_Package_Length = 9;

  // Load Data
  for(i = 0; i < Data_Length; i++)
  {
    LORA_Data[LORA_Package_Length + i] = tmpData[i];
  }

  //Add data Length to package length
  LORA_Package_Length = LORA_Package_Length + Data_Length;

  //Calculate MIC
  Calculate_MIC(LORA_Data, MIC, LORA_Package_Length, _frameCounter, Direction);

  //Load MIC in package
  for(i = 0; i < 4; i++)
  {
    LORA_Data[i + LORA_Package_Length] = MIC[i];
  }

  //Add MIC length to LORA package length
  LORA_Package_Length = LORA_Package_Length + 4;

  // Send Package
  send_Packet(LORA_Data, LORA_Package_Length);
